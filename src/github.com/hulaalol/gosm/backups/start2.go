...
type dataHandlerWays struct {
	...
	edgesOut  *[nofEdges][2]int64
	edgesOutS *[nofEdges]int8
	edgesOutD *[nofEdges]float32
	mutex     *sync.Mutex
}
...
func (d *dataHandlerWays) ReadWay(w gosmparse.Way) {
	...
	if _, ok1 := w.Element.Tags["highway"]; ok1 {
		...
		for i := 0; i < len(w.NodeIDs)-1; i++ {
			d.edgesOutS[d.wayCount] = int8(m)
			d.edgesOut[d.wayCount][0] = w.NodeIDs[i]
			d.edgesOut[d.wayCount][1] = w.NodeIDs[i+1]
			d.wayCount++
...
func decoder() {
	...
	var edgesOut [nofEdges][2]int64
	var edgesOutS [nofEdges]int8
	var edgesOutD [nofEdges]float32

	d := dataHandlerWays{0, 0, &nodes, &edgesOut, &edgesOutS, &edgesOutD, &sync.Mutex{}}
	...
	sort.Slice(edgesOut2[:], func(i, j int) bool {
		for x := range edgesOut2[i] {
			if edgesOut2[i][x] == edgesOut2[j][x] {
				continue
			}
			return edgesOut2[i][x] < edgesOut2[j][x]
		}
		return false
	})
