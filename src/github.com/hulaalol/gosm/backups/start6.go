...
var nodeRef map[int64]NodeCrumb
...
func (d *dataHandlerWays) ReadWay(w gosmparse.Way) {
	...
	if _, ok1 := w.Element.Tags["highway"]; ok1 {
		...
		for i := 0; i < len(w.NodeIDs)-1; i++ {
			d.edgesOut[d.wayCount] = Edge{w.NodeIDs[i], w.NodeIDs[i+1], 0, uint8(m), uint8(access)}
			d.wayCount++
...
func decoder() {
	...
	for i := 0; i <= len(edgesOut2)-1; i++ {
		var nodeIdxIncoming = sort.Search(len(nodes3)-1, func(k int) bool { return edgesOut2[i].n1 <= nodes3[k].id })
		offsetsIn1 = append(offsetsIn1, uint32(nodeIdxIncoming))
	}
...
func getPath(origin uint32, destiny uint32) (uint32, []uint32) {
	...
	for len(*h.values) > 0 {
		...
		//var nodeIdx = sort.Search(len(finalNodes)-1, func(k int) bool { return node <= finalNodes[k].id })
		//fmt.Println("nodeIdx")
		//fmt.Println(nodeIdx)
		// get eOut
		...
		var eIn = nodeRef[finalNodes[node].id].in

		//var edgeInd = makeRange(eOutIdxStart, eOutIdxEnd)

		log.Println(eOut)
		for _, e := range eIn {
			var tmp = finalEdgesOut[e]
			var tmpEdge = Edge{tmp.n2, tmp.n1, tmp.distance, tmp.speed, tmp.access}
			...
			if !skip {
				log.Println("add incoming edge")
				log.Println(tmpEdge)
				eOut = append(eOut, tmpEdge)
			}
		...
		for _, e := range eOut {
			...
			// if outgoing edge node has not been visited
			if !visited[nodeRef[e.n2].idx] {
				// We calculate the total spent so far plus the cost and the path of getting here
				fmt.Println("push")
				h.push(path{value: p.value + uint32(e.distance), nodes: append([]uint32{}, append(p.nodes, nodeRef[e.n2].idx)...)})
			}
		}
...
func dijkstra(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		var startNode = findClosestNode(req.StartLat, req.StartLon, 10)
		log.Println(startNode)

		var destNode = findClosestNode(req.TargetLat, req.TargetLon, 10)
		log.Println(destNode)

		var distance, path = getPath(uint32(startNode), uint32(destNode))
...
func main() {
	...
	nodeRef = make(map[int64]NodeCrumb)

	//for idx, node := range finalNodes {
	//	var eOutStart = finalOffsetsOut[idx]
	//	var eOutEnd = finalOffsetsOut[idx+1]

	//	var edgeInStart = sort.Search(len(finalOffsetsIn1)-1, func(k int) bool { return uint32(idx) <= finalOffsetsIn1[k] })
	//	var edgeInEnd = edgeInStart

	//	for finalOffsetsIn1[edgeInEnd] == finalOffsetsIn1[edgeInStart] {
	//		edgeInEnd++
	//		if edgeInEnd == len(finalOffsetsIn1) {
	//			break
	//		}
	//	}
	//	edgesIn := make([]uint32, edgeInEnd-edgeInStart)
	//	for i := edgeInStart; i < edgeInEnd; i++ {
	//		edgesIn = append(edgesIn, finalOffsetsIn2[i])
	//	}
	//	nodeRef[node.id] = NodeCrumb{uint32(idx), makeRange(eOutStart, eOutEnd), edgesIn}
	//}
