...
func decoder() {
	...
	b := [8]string{"bridleway", "escape", "raceway", "service", "proposed", "construction", "elevator", "platform"}
	for i := 0; i < len(b); i++ {
		blacklist[b[i]] = Empty{}
	}
...
var hpItems uint32 = 0

var hpMaxSize = 1000000
...
func (h *heap) push(p path) {
	...
	if hpItems == uint32(hpMaxSize) {
		fmt.Println("heap full remove last item!")

		hp.Remove(h.values, hpMaxSize)
	}
...
func getPath(origin Node, destiny Node) (uint32, []Node) {
	...
	for len(*h.values) > 0 {
		...
		p := h.pop()
		fmt.Println("\rdistance to target", p.heuristic*2.0)
		...
		if node == destinyHPN {
			...
			var result = make([]Node, len(p.nodes))
			for idx, n := range p.nodes {
				result[idx] = Node{0, n.idx, n.lat, n.lon}
			}
		...
		var eInIdxStart = finalOffsetsOut[node.idx+1]
		var eOut = finalEdgesOut[eOutIdxStart:eInIdxStart]

		if _, ok := finalEdgesIn[node.idx]; ok {
			var eInContainer = finalEdgesIn[node.idx]

			for _, eI := range eInContainer.eIn {
		...
		//fmt.Println(eOut)
		for _, e := range eOut {
			if !visited[finalNodes[e.n2idx].idx] {
				// We calculate the total spent so far plus the cost and the path of getting here
				var luftlinie = float32(geoDistancePrecision(destiny.lat, destiny.lon, finalNodes[e.n2idx].lat, finalNodes[e.n2idx].lon)) / 2.0

				var hpn = HeapNode{finalNodes[e.n2idx].idx, finalNodes[e.n2idx].lat, finalNodes[e.n2idx].lon}

				h.push(path{value: p.value + uint32(e.distance), heuristic: luftlinie, nodes: append([]HeapNode{}, append(p.nodes, hpn)...)})
			}
		}

		visited[node.idx] = true
	}
	return 0, []Node{Node{0, 0, 0, 0}}
}
...
func (data *EdgesIn) AppendOffer(eIn EdgeIn) {
	tmp := make([]EdgeIn, len(data.eIn)+1)
	for i := 0; i < len(data.eIn); i++ {
		tmp[i] = data.eIn[i]
	}
	tmp[len(data.eIn)] = eIn
	data.eIn = tmp
}
	...
	for _, e := range finalEdgesOut {
		...
		if finalNodes[i1].id == e.n1 {
			if _, ok := finalEdgesIn[e.n2idx]; ok {
				//do something here
				eIn := finalEdgesIn[e.n2idx]
				eIn.AppendOffer(EdgeIn{uint32(i1), e.distance, e.speed, e.access})
				finalEdgesIn[e.n2idx] = eIn
