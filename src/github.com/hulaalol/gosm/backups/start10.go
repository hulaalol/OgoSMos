...
func getPath(origin Node, destiny Node) (uint32, []Node) {
	...
	start := HeapNode{origin.idx, origin.lat, origin.lon}
	h.push(path{value: 0, heuristic: 0, nodes: []HeapNode{start}})

	came_from := make(map[uint32]HeapNode)
	cost_so_far := make(map[uint32]uint32)

	came_from[start.idx] = start
	...
	for len(*h.values) > 0 {
		...
		if node == destinyHPN {
			...
			path := make([]HeapNode, 0)

			for c != originHPN {
				c = HeapNode{c.idx, c.lat, c.lon}
				path = append(path, c)
				c = came_from[c.idx]
			}
		...
		for _, e := range eOut {
			...
			new_cost := cost_so_far[node.idx] + uint32(e.distance)

			exists := false

			if _, ok := cost_so_far[e.n2idx]; ok {
				exists = true
			}

			if !exists || (new_cost < cost_so_far[e.n2idx]) {
				//if new_cost < cost_so_far[e.n2idx] {

				cost_so_far[e.n2idx] = new_cost
				...
				prio := float32(new_cost) + heureka
				hpn := HeapNode{finalNodes[e.n2idx].idx, finalNodes[e.n2idx].lat, finalNodes[e.n2idx].lon}
				h.push(path{value: new_cost, heuristic: prio, nodes: append([]HeapNode{}, append(p.nodes, hpn)...)})
				came_from[e.n2idx] = p.nodes[len(p.nodes)-1]
			}
