...
func getPath(origin int64, destiny int64) (uint32, []int64) {
	...
	for len(*h.values) > 0 {
		...
		// get eOut
		var eOutIdxStart = finalOffsetsOut[nodeIdx]
		var eInIdxStart = finalOffsetsOut[nodeIdx+1]
		...
		for _, e := range eOut {
			if !visited[e.n2] {
				...
				h.push(path{value: p.value + uint32(e.distance), nodes: append([]int64{}, append(p.nodes, e.n2)...)})
			}
		}
...
func dijkstra(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		...
		var distance, path = getPath(startNode.id, destNode.id)
		...
		for idx, _ := range path {
			...
			if idx != len(path)-1 {
				...
				coords = append(coords, LeafletEdge{[]float32{finalNodes[nodeIdx1].lat, finalNodes[nodeIdx1].lon, finalNodes[nodeIdx2].lat, finalNodes[nodeIdx2].lon}})
			}
